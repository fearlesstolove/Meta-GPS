...
parser.add_argument('--dataset', default='Amazon_clothing',
                    help='Dataset:Amazon_clothing/Amazon_eletronics/dblp/CoraFull/ogb-products/reddit')
...
adj, features, labels, class_list_train, class_list_valid, class_list_test, id_by_class, adj_noloop = load_data(
...
def gnn_precompute(fea, adj_tilde, adj):
    list_mat = [fea]
    X = fea
    X_tilde = fea
    X_sum = torch.zeros_like(fea)
    alpha = F.softmax(torch.ones(2 * args.hop + 1), dim=0)
    for i in range(args.hop):
        X = torch.spmm(adj, X)
        X_tilde = torch.spmm(adj_tilde, X_tilde)
        list_mat.append(X)
        list_mat.append(X_tilde)
    for i in range(args.hop * 2 + 1):
        X_f = list_mat[i]
        X_f = torch.mul(X_f, alpha[i])
        #X_f = HopNorm(X_f)
        #X_f = torch.mul(X_f, 1 / (args.hop * 2 + 1))
        X_sum += X_f
        #list_cat.append()
    features = X_sum #/ (args.hop * 2 + 1)
    return features


features = sgc_precompute(features, adj, 2)
#features = gnn_precompute(features, adj, adj_noloop)
# embeddings = Encoder(args.hop, features.shape[1]) useless
# modify
config = [('Linear', [args.way,
                      args.hidden])]  # [('Linear', [features.shape[1], args.hidden])]   [('GraphConv', [features.shape[1], args.hidden])]

config = config + [('Linear', [args.hidden, labels_num])]

config_scal = [('Linear', [args.hidden * (args.way + 1) + labels_num * (args.hidden + 1), labels_num])]
config_trans = [('Linear', [args.hidden * (args.way + 1) + labels_num * (args.hidden + 1), labels_num])]
...
maml = Meta(args, config, config_scal, config_trans, features, labels_num, adj, adj_noloop).to(device)
...
def train(class_selected, id_support, id_query):
    ...
    acc, f1 = maml(id_support, y_spt, id_query, y_qry, meta_information, training=True)
    return acc, f1
...
def test(class_selected, id_support, id_query):
    ...
    acc, f1 = maml(id_support, y_spt, id_query, y_qry, meta_information, training=False)
    return acc, f1
    '''
    for i in range(task_num):
        y_spt[i] = torch.LongTensor([class_selected[i].index(j) for j in labels[id_support[i]]])
        y_qry[i] = torch.LongTensor([class_selected[i].index(j) for j in labels[id_query[i]]])
    acc = maml(id_support, y_spt, id_query, y_qry)
    return acc
    '''
    ...
    for episode in range(args.episodes):
        ...
        if episode > 0 and episode % 10 == 0:
            ...
            for idx in range(meta_test_num):
                ...
                meta_test_acc.append(acc_test)
                meta_test_f1.append(f1_test)
            print("Meta-Test_Accuracy: {}".format(np.array(meta_test_acc).mean(axis=0)))
            max_test_acc = np.max(np.array(meta_test_acc).mean(axis=0))
            if max_test_acc > best_test_acc:
                best_test_acc = max_test_acc
                best_episode = episode
            print("Meta-Test_F1: {}".format(np.array(meta_test_f1).mean(axis=0)))

    print("Total time elapsed: {:.4f}s".format(time.time() - t_total))
    print("{} way {} shot best test acc is {}, at {} episode".format(n_way, k_shot, best_test_acc, best_episode))